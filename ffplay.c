...
static void video_image_display(VideoState *is) {
    ...
    if (!vp->uploaded) {
        ...
        if (realloc_texture(&is->vid_texture, sdl_pix_fmt, vp->frame->width, vp->frame->height, SDL_BLENDMODE_NONE, 0) <
            0)
            return;
        ...
        if (got_frame) {
            ...
            if (reconfigure) {
                ...
                av_log(NULL, AV_LOG_DEBUG,
                       ...
                       frame->sample_rate, av_frame_get_channels(frame), av_get_sample_fmt_name(frame->format), buf2,
...
static VideoState *stream_open(const char *filename, AVInputFormat *iformat) {
    ...
    /* start video display */
    if ((&is->pictq, &is->videoq, VIDEO_PICTURE_QUEUE_SIZE, 1) < 0)
        goto fail;
